...
fun EditorRoute(viewModel: EditorViewModel = viewModel()) {
    ...
    val imagePicker = rememberLauncherForActivityResult(ActivityResultContracts.PickVisualMedia()) { it?.let(viewModel::importSource) }
    val projectSaver = rememberLauncherForActivityResult(ActivityResultContracts.CreateDocument("application/json")) { it?.let(viewModel::saveProject) }
    val projectOpener = rememberLauncherForActivityResult(ActivityResultContracts.OpenDocument()) { it?.let(viewModel::openProject) }
    val recipeSaver = rememberLauncherForActivityResult(ActivityResultContracts.CreateDocument("application/json")) { it?.let(viewModel::saveRecipe) }
    val recipeOpener = rememberLauncherForActivityResult(ActivityResultContracts.OpenDocument()) { it?.let(viewModel::openRecipe) }
    val pngExporter = rememberLauncherForActivityResult(ActivityResultContracts.CreateDocument("image/png")) { it?.let(viewModel::exportPng) }
...
private fun EditorScreen(
    ...
    Surface(Modifier.fillMaxSize()) {
        Column(Modifier.fillMaxSize()) {
            Toolbar(state, pickImage, saveProject, openProject, saveRecipe, openRecipe, exportPng, viewModel)
                ...
                if (maxWidth >= 900.dp) {
                    ...
                    Column(Modifier.fillMaxSize()) {
                        ...
                        Row(Modifier.weight(1f).fillMaxWidth()) {
                            StackPanel(state, viewModel, Modifier.weight(1f).fillMaxHeight())
                            Inspector(state, viewModel, Modifier.weight(1f).fillMaxHeight())
                        }
                    }
                }
            }
...
private fun Toolbar(
    ...
    Row(
        ...
        Button(onClick = exportPng, enabled = state.project.source != null && !state.operationInProgress) { Text("EXPORT PNG") }
        ...
        if (state.rendering || state.operationInProgress) CircularProgressIndicator(Modifier.width(24.dp))
    }
}
...
private fun Status(text: String, error: Boolean, clear: () -> Unit) {
    Row(
        Modifier.fillMaxWidth().background(if (error) MaterialTheme.colorScheme.errorContainer else MaterialTheme.colorScheme.secondaryContainer).clickable(onClick = clear).padding(8.dp),
    ...
    Column(modifier.padding(12.dp)) {
        ...
        Row(Modifier.fillMaxWidth().horizontalScroll(rememberScrollState()).padding(vertical = 8.dp), horizontalArrangement = Arrangement.spacedBy(6.dp)) {
            BuiltInEffects.registry.all().forEach { definition ->
                OutlinedButton(onClick = { viewModel.addEffect(definition.type) }) { Text("+ ${definition.displayName}") }
            }
        }
...
@Composable
private fun EffectRow(effect: EffectInstance, state: EditorUiState, viewModel: EditorViewModel) {
    val selected = state.selectedEffectId == effect.id
    Column(
        Modifier.fillMaxWidth().background(if (selected) MaterialTheme.colorScheme.primaryContainer else MaterialTheme.colorScheme.surfaceVariant).clickable { viewModel.select(effect.id) }.padding(8.dp)
    ) {
        Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.SpaceBetween) {
            Text(BuiltInEffects.registry.definition(effect.type)?.displayName ?: "UNRESOLVED: ${effect.type}")
            Switch(checked = effect.enabled, onCheckedChange = { viewModel.setEnabled(effect.id, it) }, enabled = effect.isResolved)
        }
        Row(horizontalArrangement = Arrangement.spacedBy(4.dp)) {
            OutlinedButton(onClick = { viewModel.moveEffect(effect.id, -1) }) { Text("↑") }
            OutlinedButton(onClick = { viewModel.moveEffect(effect.id, 1) }) { Text("↓") }
            OutlinedButton(onClick = { viewModel.setSolo(if (state.project.soloEffectId == effect.id) null else effect.id) }) { Text(if (state.project.soloEffectId == effect.id) "UNSOLO" else "SOLO") }
...
private fun Viewport(state: EditorUiState, modifier: Modifier) {
    Box(modifier.background(MaterialTheme.colorScheme.surfaceVariant), contentAlignment = Alignment.Center) {
        ...
        if (bitmap == null) Text("OPEN AN IMAGE") else Image(bitmap.asImageBitmap(), null, Modifier.fillMaxSize().padding(12.dp), contentScale = ContentScale.Fit)
        state.lastRenderDurationMillis?.let { Text("${"%.1f".format(it)} ms", Modifier.align(Alignment.BottomEnd).padding(12.dp)) }
    }
}
...
private fun Inspector(state: EditorUiState, viewModel: EditorViewModel, modifier: Modifier) {
    ...
    Column(modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        Slider(value = effect.opacity.toFloat(), onValueChange = { viewModel.setOpacity(effect.id, it.toDouble()) }, valueRange = 0f..1f)
        val definition = BuiltInEffects.registry.definition(effect.type)
        definition?.parameters?.forEach { parameter ->
            when (parameter) {
                is ParameterDefinition.Integer -> {
                    val current = (effect.parameters[parameter.key] as? ParameterValue.Integer)?.value ?: parameter.default.value
                    Text("${parameter.label} $current")
                    Slider(
                        value = current.toFloat(),
                        onValueChange = { viewModel.setParameter(effect.id, parameter.key, ParameterValue.Integer(it.roundToInt())) },
                        valueRange = parameter.minimum.toFloat()..parameter.maximum.toFloat(),
                        steps = ((parameter.maximum - parameter.minimum) / parameter.step - 1).coerceAtLeast(0),
                    )
                }
                else -> Text("${parameter.label}: control pending")
            }
        }
        if (!effect.isResolved) Text("This node is preserved but cannot execute in this build.")
    }
}
