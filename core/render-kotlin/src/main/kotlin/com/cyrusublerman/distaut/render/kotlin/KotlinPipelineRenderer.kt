...
class KotlinPipelineRenderer : RenderBackend {
    override fun render(request: RenderRequest): RenderResult {
        ...
        for (effect in request.effects) {
            ...
            if (effect.opaquePayload != null) throw UnsupportedEffectException(effect.id, effect.type)
            if (effect.blendMode != "normal") throw UnsupportedBlendModeException(effect.id, effect.blendMode)
            ...
            current = if (effect.opacity >= 1.0) processed else NormalOpacityBlend.apply(current, processed, effect.opacity, request)
            applied += effect.id
        }
        return RenderResult(request.generation, request.sourceRevision, request.quality, current, applied, System.nanoTime() - started)
    }
}
...
object GreyscaleKernel {
    fun apply(source: PixelBuffer, request: RenderRequest? = null): PixelBuffer {
        ...
        while (i < output.size) {
            ...
            output[i] = y; output[i + 1] = y; output[i + 2] = y
            i += 4
        }
...
object PosteriseKernel {
    fun apply(source: PixelBuffer, parameter: ParameterValue?, request: RenderRequest? = null): PixelBuffer {
        val levels = when (parameter) { is ParameterValue.Integer -> parameter.value; is ParameterValue.Decimal -> parameter.value.roundToInt(); else -> 6 }.coerceIn(2, 256)
...
object NormalOpacityBlend {
    fun apply(base: PixelBuffer, processed: PixelBuffer, opacity: Double, request: RenderRequest? = null): PixelBuffer {
        ...
        val output = base.rgba.copyOf(); var i = 0
        while (i < output.size) {
            if (i and 0x3fff == 0) request?.let(::checkCancelled)
            for (channel in 0..2) {
